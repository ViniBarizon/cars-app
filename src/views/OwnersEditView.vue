  ...
  <div>
    ...
    <div class="card">
      <form @submit.prevent="store">
        ...
        <button class="delete-button" type="button" @click="destroy">Excluir</button>
      </form>
    </div>
  </div>
</template>
...
onMounted(async () => {
  ...
  const id = useRoute().params.ownerId
  fetchOwnerById(id)
  try {
...
async function fetchOwnerById(id) {
  try {
    const { data } = await http.get(`/owners/${id}`)
    owner.id = data.id
    owner.name = data.name
    owner.birth_date = data.birth_date
    owner.gender = data.gender
    owner.cpf_cnpj = data.cpf_cnpj
    owner.email = data.email
    owner.address_zip_code = data.address_zip_code
    owner.address_street = data.address_street
    owner.address_number = data.address_number
    owner.address_block = data.address_block
    owner.address_city = data.address_city
    owner.address_state = data.address_state
  } catch (error) {
    console.log(error)
  }
}
...
.delete-button {
  background-color: #a3170d !important;
  cursor: pointer;
}
</style>
